...
const showData = (obj) => {
  ...
  const feelsLike = createElement(
    "p",
    `Feels like: ${convertToF(obj.feels_like)}°F`,
    detailsArea
  );
  createElement("p", `Humidity: ${obj.humidity}%`, detailsArea);
  const highLow = createElement(
    "p",
    `High/Low: ${convertToF(obj.temp_max)}°F/${convertToF(obj.temp_min)}°F`,
    detailsArea
  );
  ...
  fButton.addEventListener("click", () => {
    ...
    highLow.textContent = `High/Low: ${convertToF(obj.temp_max)}°F/${convertToF(
      obj.temp_min
    )}°F`;
  });
  ...
  cButton.addEventListener("click", () => {
    ...
    highLow.textContent = `High/Low: ${convertToC(obj.temp_max)}°C/${convertToC(
      obj.temp_min
    )}°C`;
  });
};
